...
#  form.Checkbox('reenviar'),
  form.Dropdown('yourProfile', ['Equilibrated tourist','Tan-aholic','The cheaper the better','Only one live','Keep me fresh'],description="Your tourist profile",\
  post="how would you describe your self?", size="5")
  )


def Respuesta(ciudadSalida,firstPossibleDepartureDate,lasttPossibleDepartureDate,duration,currency,profile):
    ...
    df=pd.DataFrame(list(map(list, zip (listadoDestinosNfila,listadoDestinosLargo,listadoDestinosCodigo,listadoDestinosFechaSalida,
     listadoDestinosFechaVuelta,listadoDestinosPrecioVuelo,listadoDestinosLats,listadoDestinosLons,
     listadoDestinosPais,listadoDestinosPrecioHoteles,listadoBigMacIndex,listadoIndex,
     listadoPrecAverage,listadoPrecRange,listadoPrecProb,listadoCloudCovAverage,listadoCloudCovRange,
     listadoCloudCovProb,ListadoTempMaxAverage,ListadoTempMaxRange,ListadoTempMinAverage,ListadoTempMinRange))))

    df.columns=["Destination Code","Destination","Code","Departure","Return","Flight Price","Lat","Lon","Country","Hotel Price","BigMacIndex","yourProfile",
                "Total Precipitation","Range","Rain Probability","Cloud Coverage","range","% of cloudy days",
                "Max Temp","Range","Min Temp","Range"]    
...
# Método POST
  def POST(self):
     ...
     if not form.validates():
       ...
       return Respuesta(form['departureCity'].value,form['firstPossibleDepartureDate'].value,form['lasttPossibleDepartureDate'].value,form['duration'].value,form['currency'].value,form['yourProfile'].value)
